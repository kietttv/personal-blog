...
if (isset($_GET['id'])) {
    ...
    $selectComment = "SELECT * FROM `comment` WHERE postID = '$id' AND stt = 0";
    $reComment = mysqli_query($conn, $selectComment);

    $selectCount = "SELECT COUNT(id) FROM `comment` WHERE postID = '$id' AND stt = 0";
                    ...
                    <div class="media mb-4">
                        ...
                        <div class="media-body">
                            ...
                            <?php if (isset($_SESSION['user'])) { ?>
                                <div>
                                    <button class="btn btn-sm btn-light" type="submit" id="hide" name="hide" onclick="hideComment(<?= $rowComment['id']?>)">Hide</button>
                                </div>
                                <script>
                                    function hideComment(id) {
                                        if (confirm('Do you want to hide comment [' + id + ']?') == true) {
                                            window.location.href = 'hideComment.php?id=' + id;
